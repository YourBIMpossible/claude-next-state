...
- 2026-09-02 (model routing 1B deploy, #548 8cad22b5): post-deploy log check per `docs/engineering/deployments.md` not yet done — inspect `model_resolved`/`model_selection_denied` structured events for the fail-open regression signature (platform-route traffic right after a `credential_unavailable` denial for the same firm/window). [WANDER-ROUTING-1B-FAILOPEN-LOGCHECK]
- 2026-09-23 (review-remediation close): owner cleanup — merged lane leftovers: remote branches claude/docref-collisions, claude/g4-docs-budget, claude/breach-chain-xref (WS), claude/review-remediation; worktrees AddIns g4-docs-budget + review-remediation, WS breach-chain-xref. All merged (#159, #160, #161, WS #157). DONE 2026-09-23: all pruned.
